...
struct ExtendedPair {
  tuple<int, int, int, int> position; // top-left row, top-left col,
...
vector<vector<uint16_t>> get_random_board(int n) {
  ...
  cpr::Response r =
      cpr::Post(cpr::Url{"https://naprock-server.vercel.app/getBoard"},
  ...
  for (int i = 0; i < n; ++i)
    for (int j = 0; j < n; ++j)
      board[i][j] =
          static_cast<uint16_t>(responseData["board"][i][j].get<int>());
...
vector<vector<uint16_t>> rotate_submatrix(vector<vector<uint16_t>> grid, int k,
                                          int i, int j) {
...
vector<vector<uint8_t>> rotate_submatrix_u8(vector<vector<uint8_t>> grid, int k,
                                            int i, int j) {
...
vector<vector<uint16_t>> apply_rotations(vector<vector<uint16_t>> grid,
                                         const vector<Rotation> &path) {
...
bool Check_Valid(int i, int j, int k) {
  if (locked[i][j] || locked[i + k][j + k] || locked[i][j + k] ||
      locked[i + k][j])
    return false;
  for (int ft = 1; ft < k; ft += 1)
    if (locked[i + ft][j] || locked[i][j + ft] || locked[i + ft][j + k] ||
        locked[i + k][j + ft])
      return false;
  return true;
}
...
pair<int, int> find_pos(const vector<vector<uint16_t>> &grid, int row,
                        int col) {
  if (grid.empty() || row < 0 || col < 0 || row >= (int)grid.size() ||
      col >= (int)grid[0].size())
    return {-1, -1};
...
vector<tuple<int, int, int, int, uint16_t>>
find_free_pairs(const vector<vector<uint16_t>> &grid, int track) {
  ...
  for (int j = track; j < N; ++j) {
    for (int i = N / 2 - 3; i < N; ++i) {
      if (locked[cnt + i][cnt + j])
        continue;
      if (j + 1 < N && !locked[cnt + i][cnt + j + 1] &&
          grid[i][j] == grid[i][j + 1])
        pairs.push_back({i, j, i, j + 1, grid[i][j]});
      if (i + 1 < N && !locked[cnt + i + 1][cnt + j] &&
          grid[i][j] == grid[i + 1][j])
...
move_free_pair_to_target(const vector<vector<uint16_t>> &initial_grid, int pr1,
                         int pc1, int pr2, int pc2, int target_r, int target_c,
                         bool is_vertical) {
  ...
  for (int depth = 0; depth < max_depth; ++depth) {
    ...
    for (const auto &cur : current_beam) {
      ...
      if (is_vertical)
        at_target =
            (cur.grid[target_r][target_c] == cur.grid[target_r + 1][target_c] &&
             cur.grid[target_r][target_c] == initial_grid[pr1][pc1]);
      else
        at_target =
            (cur.grid[target_r][target_c] == cur.grid[target_r][target_c + 1] &&
             cur.grid[target_r][target_c] == initial_grid[pr1][pc1]);
          ...
          for (int r = 0; r <= N - k; ++r)
            for (int c = 0; c <= N - k; ++c) {
              ...
              if (r <= pr2 && pr2 < r + k && c <= pc2 && pc2 < c + k)
                affects = true;
              if (r <= target_r && target_r < r + k && c <= target_c &&
                  target_c < c + k)
                affects = true;
              if (!affects)
                continue;
              ...
              local_beam.push_back(
                  {new_grid,
                   new_path,
                   {r - c + cur.pos.second, r + c + k - 1 - cur.pos.first}});
            }
      }
#pragma omp critical
      {
    ...
    sort(next_beam.begin(), next_beam.end(),
         [&](const State &a, const State &b) {
           int da = abs(a.pos.first - target_r) + abs(a.pos.second - target_c) +
                    2 * a.path.size();
           int db = abs(b.pos.first - target_r) + abs(b.pos.second - target_c) +
                    2 * b.path.size();
           return da < db;
         });
...
search_pair(const vector<vector<uint16_t>> &initial_grid, int row1, int col1,
  ...
  vector<State> current_beam = {
      {initial_grid, {}, find_pos(initial_grid, row1, col1)}};
...
#pragma omp for schedule(dynamic) nowait
      for (int beam_idx = 0; beam_idx < (int)current_beam.size(); ++beam_idx) {
        const auto &cur = current_beam[beam_idx];
        for (int k = min(
                 max(abs(cur.pos.first - row2), abs(cur.pos.second - col2)) + 2,
                 24);
             k >= 2; --k)
          for (int r = mnr; r <= mxr + 1 - k; ++r)
            for (int c = mnc; c <= mxc + 1 - k; ++c) {
              if ((r <= row1 && row1 < r + k && c <= col1 && col1 < c + k) ||
                ...
                continue;
              bool affects =
                  (r <= row2 && row2 < r + k && c <= col2 && col2 < c + k);
              if (!affects)
                continue;
              vector<vector<uint16_t>> new_grid =
                  rotate_submatrix(cur.grid, k, r, c);
...
#pragma omp critical
    ...
    sort(next_beam.begin(), next_beam.end(),
         [&](const State &a, const State &b) {
           int da = abs(a.pos.first - row2) + abs(a.pos.second - col2) +
                    2 * a.path.size();
...
ExtendedPair extend_free_pair(const vector<vector<uint16_t>> &initial_grid,
                              int pr1, int pr2, int pc1, int pc2, bool tl,
                              bool bl, bool tr, bool br, bool is_vertical) {
  ...
  if (is_vertical) {
    if (tl && pc1 - 1 >= 0) {
      ...
      temp = search_pair(initial_grid, pr2, pc1 - 1, pr1, pc1 - 1, 0, N - 1, 0,
                         N - 1);
    ...
    if (bl && pc1 - 1 >= 0) {
      locked[pr1 + cnt][pc1 + cnt] = 1;
      locked[pr2 + cnt][pc2 + cnt] = 1;
      temp = search_pair(initial_grid, pr1, pc1 - 1, pr2, pc1 - 1, 0, N - 1, 0,
                         N - 1);
      locked[pr1 + cnt][pc1 + cnt] = 0;
      locked[pr2 + cnt][pc2 + cnt] = 0;
      if (temp.first < result.cost) {
    ...
    if (tl && pr1 - 1 >= 0) {
      locked[pr1 + cnt][pc1 + cnt] = 1;
      locked[pr2 + cnt][pc2 + cnt] = 1;
      temp = search_pair(initial_grid, pr1 - 1, pc2, pr1 - 1, pc1, 0, N - 1, 0,
                         N - 1);
      locked[pr1 + cnt][pc1 + cnt] = 0;
      locked[pr2 + cnt][pc2 + cnt] = 0;
      if (temp.first < result.cost) {
    ...
    if (tr && pr1 - 1 >= 0) {
      locked[pr1 + cnt][pc1 + cnt] = 1;
      locked[pr2 + cnt][pc2 + cnt] = 1;
      temp = search_pair(initial_grid, pr1 - 1, pc1, pr1 - 1, pc2, 0, N - 1, 0,
                         N - 1);
      locked[pr1 + cnt][pc1 + cnt] = 0;
      locked[pr2 + cnt][pc2 + cnt] = 0;
      if (temp.first < result.cost) {
        ...
        for (int k = 2; k <= N - 1; ++k)
          for (int r = 0; r <= N - k; ++r)
            for (int c = 0; c <= N - k; ++c) {
              ...
              if (c_tlr >= r && c_tlc >= c && c_tlr + 1 < r + k &&
                  c_tlc + 1 < c + k)
                affects = true;
              if (!affects)
                continue;
              ...
              local_beam.push_back(
                  {new_grid,
                   new_path,
                   {r - c + cur.pos.second, r + c + k - 2 - cur.pos.first}});
            }
      }
#pragma omp critical
      {
...
bool sortBySecond(tuple<int, int, int, int, uint16_t> &a,
                  tuple<int, int, int, int, uint16_t> &b) {
  if (get<1>(a) < get<1>(b))
    return true;
  return get<0>(a) < get<0>(b);
}
pair<int, vector<Rotation>> Vertical_place(vector<vector<uint16_t>> grid,
                                           int Fsize, int j) {
  int row = Fsize / 2 - 2;
  int min_ops = 999;
  ...
  pair<int, vector<Rotation>> temp_d =
      search_pair(grid, row, j, row + 1, j, 0, Fsize - 1, 0, Fsize - 1);
    ...
    for (const auto &[pr1, pc1, pr2, pc2, val] : free_pairs) {
      ...
      if (pr1 == pr2 && min_col >= j) {
        ...
        if (r < 0 || j + k >= Fsize || k <= 1)
          continue;
        if (locked[r + cnt][j + cnt + k - 1] ||
            locked[r + cnt + k - 1][j + cnt + k - 1])
          continue;
        partial_result.clear();
        partial_result.emplace_back(k, r, j);
        cout << "Using free pair (" << pr1 << "," << pc1 << ")-(" << pr2 << ","
             << pc2 << ")";
  ...
  for (int step = j; step <= Fsize / 2 - 4 + j; step++) {
    for (int i = row + 1; i <= min(row + Fsize - 3 - step, Fsize - 3); i++) {
      auto [ops1, path1] =
          search_pair(grid, i, step + 1, i, step, 0, Fsize - 1, 0, Fsize - 1);
      if (ops1 != 1) {
        auto [ops1t, path1t] =
            search_pair(grid, i, step, i, step + 1, 0, Fsize - 1, 0, Fsize - 1);
        if (ops1t != 1)
          continue;
        ops1 = ops1t;
        path1 = path1t;
...
pair<int, vector<Rotation>> Horizontal_place(vector<vector<uint16_t>> grid,
                                             int Fsize, int j) {
  int row = Fsize / 2 - 2;
  int min_ops = 999;
  ...
  if (SET_FP) {
    for (const auto &[pr1, pc1, pr2, pc2, val] : free_pairs) {
      ...
      bool is_significant = !(
          locked[row + cnt][cnt + pc1] || locked[row + cnt][cnt + pc2] ||
      ...
      if (is_significant) {
        ...
        if (is_vertical_pair) {
          tl = (pr1 >= 0 && pc1 - 1 >= 0 && !locked[pr1 + cnt][pc1 - 1 + cnt] &&
          ...
          br = (pr2 < Fsize && pc1 + 1 < Fsize &&
                ...
                !locked[pr2 + cnt][pc1 + 1 + cnt]);
          if (tl || tr || bl || br)
            extend_result = extend_free_pair(grid, pr1, pr2, pc1, pc2, tl, bl,
                                             tr, br, true);
        } else {
          tl = (pr1 - 1 >= 0 && pc1 >= 0 && !locked[pr1 - 1 + cnt][pc1 + cnt] &&
        ...
        if (tlr != -1 && tlc != -1 && brr != -1 && brc != -1) {
          auto move_result =
              move_extend_free_pair(apply_rotations(grid, extend_result.path),
                                    tlr, tlc, brr, brc, row, j);
          extend_result.path.insert(extend_result.path.end(),
                                    move_result.second.begin(),
                                    move_result.second.end());
          ...
          if (extend_result.cost < min_ops) {
            ...
            cout << "Using extend free pair (" << tlr << "," << tlc << ")-("
                 << brr << "," << brc << ") with cost " << min_ops << ". ";
            ext_pair = true;
          }
        }
      }
  ...
  // First 1
  {
    ...
    for (const auto &[pr1, pc1, pr2, pc2, val] : free_pairs) {
      auto move_result =
          move_free_pair_to_target(grid, pr1, pc1, pr2, pc2, row, j, false);
    ...
    auto [ops1, path1] =
        search_pair(grid, row, j, row, j + 1, 0, Fsize - 1, 0, Fsize - 1);
  ...
  // First 2
  {
    ...
    for (const auto &[pr1, pc1, pr2, pc2, val] : free_pairs) {
      auto move_result =
          move_free_pair_to_target(grid, pr1, pc1, pr2, pc2, row + 1, j, false);
      if (move_result.first < mn_free) {
        temp_result = move_result.second;
        mn_free = move_result.first;
      }
    }
    auto [ops1, path1] = search_pair(grid, row + 1, j, row + 1, j + 1, 0,
                                     Fsize - 1, 0, Fsize - 1);
    ...
    auto [ops2, path2] = search_pair(apply_rotations(grid, path1), row, j, row,
                                     j + 1, 0, Fsize - 1, 0, Fsize - 1);
    if (ops1 + ops2 < min_ops) {
      min_ops = ops1 + ops2;
      partial_result = path1;
      partial_result.insert(partial_result.end(), path2.begin(), path2.end());
  ...
  int up = (!locked[cnt][cnt + Fsize / 2]) * 2,
      down = (!locked[cnt + Fsize - 1][cnt + Fsize / 2]) *
             (2 + (j >= row - 1) * (row - j - 2));
  if (j == row + 1)
    down = -1;
  ...
  // Down 1
  {
    ...
    for (int step = row + 1; step < Fsize - 3 + down; step++) {
      auto [ops1, path1] =
          search_pair(grid, step, j, step + 1, j, 0, Fsize - 1, 0, Fsize - 1);
      if (ops1 >= 2) {
        auto [ops1t, path1t] =
            search_pair(grid, step + 1, j, step, j, 0, Fsize - 1, 0, Fsize - 1);
        if (ops1t < ops1) {
          ops1 = ops1t;
          path1 = path1t;
        }
      ...
      auto [ops2, path2] = search_pair(grid_temp, step, j + 1, step + 1, j + 1,
                                       0, Fsize - 1, 0, Fsize - 1);
      if (ops2 >= 2) {
        auto [ops2t, path2t] = search_pair(grid_temp, step + 1, j + 1, step,
                                           j + 1, 0, Fsize - 1, 0, Fsize - 1);
        if (ops2t < ops2) {
          ops2 = ops2t;
          path2 = path2t;
  ...
  // Down 2
  {
    SET_FP = false;
    for (int step = row + 1; step < Fsize - 3 + down; step++) {
      auto [ops1, path1] = search_pair(grid, step, j + 1, step + 1, j + 1, 0,
                                       Fsize - 1, 0, Fsize - 1);
      if (ops1 >= 2) {
      ...
      auto grid_temp = apply_rotations(grid, path1);
      auto [ops2, path2] = search_pair(grid_temp, step, j, step + 1, j, 0,
                                       Fsize - 1, 0, Fsize - 1);
      if (ops2 >= 2) {
        auto [ops2t, path2t] = search_pair(grid_temp, step + 1, j, step, j, 0,
                                           Fsize - 1, 0, Fsize - 1);
        if (ops2t < ops2) {
          ops2 = ops2t;
          path2 = path2t;
  ...
  // Down 3
  {
    SET_FP = false;
    for (int step = row + 1; step < Fsize - 3 + down; step++) {
      auto [ops1, path1] = search_pair(grid, step + 1, j, step + 1, j + 1, 0,
                                       Fsize - 1, 0, Fsize - 1);
      if (ops1 >= 2) {
      ...
      locked[cnt + step + 1][cnt + j] = 1;
      locked[cnt + step + 1][cnt + j + 1] = 1;
      auto [ops2, path2] = search_pair(apply_rotations(grid, path1), step, j,
                                       step, j + 1, 0, Fsize - 1, 0, Fsize - 1);
      if (ops1 + ops2 + 1 < min_ops) {
        min_ops = ops1 + ops2 + 1;
        partial_result = path1;
        partial_result.insert(partial_result.end(), path2.begin(), path2.end());
...
int weighted_free_pairs(const vector<vector<uint16_t>> &crop, int Fsize,
                        int local_cnt) {
  int N = crop.size();
  int half = N / 2;
  ...
  auto get_weight = [&](int row, int col, int mode) -> int {
    ...
    // Check column zones
    bool is_center =
        (col >= half - 2 + local_cnt && col <= half + 1 + local_cnt);
...
#pragma omp critical
    ...
    if (beam[0].score > best_score) {
      ...
      best_seen = {beam[0]};
      cout << "[PreBeam] Depth " << depth + 1 << ": improved score to "
           << best_score << "\n";
    } else {
...
STEP_Do(int Fsize, vector<vector<uint16_t>> grid, int mode) {
  ...
  vector<pair<vector<vector<uint16_t>>, vector<Rotation>>> result(
      half + 1, {grid, pre_path});

  pair<int, vector<Rotation>> V_result =
      Vertical_place(result[mode].first, Fsize, mode);
  ...
  for (int j = mode + 2; j <= half; j += 1) {
    ...
    pair<int, vector<Rotation>> H_result =
        Horizontal_place(result[j - 2].first, Fsize, j - 2);
    int H_ops = dp[j - 2] + H_result.first;
    locked[cnt + row][cnt + j - 2] = 1;
    locked[cnt + row + 1][cnt + j - 2] = 1;

    pair<int, vector<Rotation>> V_r =
        Vertical_place(result[j - 1].first, Fsize, j - 1);
    ...
    //             for (const auto &rot : pre_path)
    //                 local_path.emplace_back(rot.k, rot.i - cnt, rot.j - cnt);
    //             result[j].first = apply_rotations(result[j].first,
    //             local_path); result[j].second.insert(result[j].second.end(),
    //             pre_path.begin(), pre_path.end());
    //             // Record in partial_path with GLOBAL coords
    //             cout << " Applied " << pre_path.size() << " rotations\n";
    //         }
    //         cout << "==========================================\n\n";
...
vector<GridState> unstuck_healing(const GridState &stuck_state,
  ...
  cout << "HEALING: Best healed state has " << healed_states[0].paired_count
       << " pairs" << endl;
...
vector<Rotation> beam_search(const vector<vector<uint16_t>> &inner_grid,
                             int offset, int max_depth) {
  ...
  GridState global_best = {
      inner_grid, {}, initial_paired, initial_heuristic, initial_hash};
  int depth = 0;
  int stuck_counter = 0;
  ...
  cout << "Beam search done. Best paired: " << global_best.paired_count << "/"
       << target_paired << "\n";
