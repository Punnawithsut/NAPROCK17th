...
vector<Rotation> beam_search(const vector<vector<int>> &initial_grid, int max_depth, double time_budget)
{
    ...
    float prev_best_score_in_depth = -1000.0f;
    ...
    for (; depth < max_depth && !beam.empty(); ++depth)
    {
        ...
        if (abs(best_score_in_depth - prev_best_score_in_depth) < 0.1f)
        {
            same_best_depth_streak++;
            cout << "Score plateau detected (" << same_best_depth_streak << " times)\n";

            if (same_best_depth_streak >= 6) // Reduced threshold
            {
                cout << "[Beam] Score plateaued for " << same_best_depth_streak
                     << " depths — triggering SA early.\n";
                g_trigger_SA = true;
                g_beam_best_grid = global_best.grid;
                g_beam_best_path = global_best.path;
                return global_best.path;
