...
int weighted_beam_DO(const vector<vector<uint16_t>> &crop, int Fsize, int PD)
{
    ...
    {
        if (locked[cnt + mid_row_end + k][cnt + CUR_J])
            continue;
        sr.insert(crop[mid_row_end + k][CUR_J]);
        if (locked[cnt + mid_row_end + k][cnt + CUR_J + 1])
            continue;
        sl.insert(crop[mid_row_end + k][CUR_J + 1]);
    }
    for (int k = 0; k < (N - CUR_J) / 2; k++)
    {
        if (locked[cnt + mid_row_start][cnt + CUR_J + k])
            continue;
...
vector<Rotation> Do_step_beam_search(const vector<vector<uint16_t>> &crop,
                                     ...
                                     GPUBeamSearch &gpu_search)
{
    const int MAX_DEPTH = Fsize;           // original maximum depth
    const int BEAM_WIDTH = 125 + 10 * cnt; // original beam width
    const double ALPHA = 0.1;              // penalty factor (idea 2)

    // Iterative deepening settings (idea 6)
    ...
    // Effective score with path‑length penalty (idea 2)
    auto effective = [&](const CropState &s) -> double
    {
        return s.score - ALPHA * s.path.size();
    };

    // Global best across all depth limits
    CropState global_best = {crop, {}, init_score};

    // ─── Iterative deepening loop ─────────────────────────────────────────
    for (int depth_limit = START_DEPTH; depth_limit <= MAX_DEPTH; depth_limit += DEPTH_STEP)
    {
...
#pragma omp parallel
                {
                    vector<CropState> local_candidates;
#pragma omp for schedule(dynamic) nowait
                    for (int bi = 0; bi < (int)beam.size(); ++bi)
                    {
                        const auto &cur = beam[bi];
                        for (int k = 2; k <= min(12, N - 1); ++k)
                            for (int r = 0; r <= N - k; ++r)
                                for (int c = 0; c <= N - k; ++c)
                                {
                                    ...
                                    auto new_path = cur.path;
                                    new_path.emplace_back(k, r, c);
                                    local_candidates.push_back({new_grid, new_path, new_score});
                                }
                    }
#pragma omp critical
                    {
            ...
            // If a solution was found at this depth, return the shortest one
            if (solution_found)
            {
                auto best_sol = *min_element(solutions.begin(), solutions.end(),
                                             [](const CropState &a, const CropState &b)
                                             {
                                                 return a.path.size() < b.path.size();
                                             });
                cout << "[DoBeam] Solution found at depth " << depth + 1
                     << " with path length " << best_sol.path.size() << "\n";
                vector<Rotation> global_path;
                for (auto &rot : best_sol.path)
                    global_path.emplace_back(rot.k, rot.i + cnt, rot.j + cnt);
                return global_path;
            }
        } // end depth loop
    ...
    cout << "[DoBeam] No solution found. Returning best path with score "
         << global_best.score << " and length " << global_best.path.size() << "\n";
    vector<Rotation> global_path;
    for (auto &rot : global_best.path)
        global_path.emplace_back(rot.k, rot.i + cnt, rot.j + cnt);
    return global_path;
}
